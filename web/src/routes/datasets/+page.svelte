<script lang="ts">
	...
	async function createDataset() {
		...
		try {
			...
			newName = '';
			newDescription = '';
			notifications.success($t('datasets.datasetCreated'));
			goto(`/datasets/${ds.id}`);
		} catch (e: any) {
	...
	async function deleteDataset(id: string) {
		...
		try {
			...
			await loadDatasets();
			notifications.success($t('datasets.datasetDeleted'));
		} catch (e: any) {
...
<svelte:head>
	<title>{$t('datasets.title')}</title>
</svelte:head>

<div class="max-w-7xl mx-auto">
	<div class="flex items-center justify-between mb-6">
		<h1 class="text-2xl font-bold text-gray-900">{$t('datasets.heading')}</h1>
		<button
	...
	{#if showCreate}
		<div class="bg-white rounded-lg border border-gray-200 p-6 mb-6">
			<h2 class="text-lg font-semibold text-gray-900 mb-4">{$t('datasets.createDataset')}</h2>
			<div class="space-y-4">
				<div>
					<label for="ds-name" class="block text-sm font-medium text-gray-700">{$t('common.name')}</label>
					<input
						id="ds-name"
						type="text"
						bind:value={newName}
				...
				<div>
					<label for="ds-desc" class="block text-sm font-medium text-gray-700">{$t('common.descriptionOptional')}</label>
					<textarea
						id="ds-desc"
	...
	{#if loading}
		<div class="text-center py-12 text-gray-500">{$t('datasets.loadingDatasets')}</div>
	...
	{:else if datasets.length === 0}
		<div class="text-center py-12 bg-white rounded-lg border border-gray-200">
			...
			<h3 class="mt-2 text-sm font-medium text-gray-900">{$t('datasets.noDatasets')}</h3>
			<p class="mt-1 text-sm text-gray-500">{$t('datasets.noDatasetsDesc')}</p>
		</div>
	{:else}
			...
			<table class="min-w-full divide-y divide-gray-200">
				<thead class="bg-gray-50">
					<tr>
						...
						<th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{$t('common.description')}</th>
					...
					{#each datasets as ds}
						<tr class="hover:bg-gray-50">
							...
							<td class="px-4 py-3 text-right">
								<button onclick={() => deleteDataset(ds.id)} class="text-sm text-red-600 hover:text-red-800">{$t('common.delete')}</button>
							</td>
						</tr>
					{/each}
				</tbody>
